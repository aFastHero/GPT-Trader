...
from prompt import MARKET_SYSTEM, MARKET_USER, OPTIONS_SYSTEM, OPTIONS_USER, DAY_TRADING_SYSTEM, DAY_TRADING_USER
...
def main(prompt_only=False):
    ...
    for symbol in symbols:
        ...
        prompt = DAY_TRADING_USER.replace('$C', insight_prompts)

        with open('prompt.txt', 'w', encoding='utf-8') as f:
            f.write(prompt)

        if not prompt_only:
            response = ask_gpt(DAY_TRADING_SYSTEM, prompt, model='gpt-4-1106-preview')
